...
export function openModal(videoId, startTime, fps) {
    ...
    Object.assign(timelinePreview.style, {
        display: 'none', position: 'absolute', bottom: '120%', 
    ...
    if (Hls.isSupported()) {
        previewHls = new Hls({ maxBufferLength: 1, maxMaxBufferLength: 2, enableWorker: true });
    ...
    const runHoverPreview = () => {
        ...
        const onSeeked = () => {
            ...
            } catch (err) { previewImg.style.display = 'none'; }
        };
    ...
    const handleTimelineClick = (e) => {
        ...
        const rect = timelineBar.getBoundingClientRect();
        const percent = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width));
        elements.modalVideoPlayer.currentTime = percent * elements.modalVideoPlayer.duration;
    };

    const updateProgress = () => {
        if (!elements.modalVideoPlayer.duration) return;
        const p = (elements.modalVideoPlayer.currentTime / elements.modalVideoPlayer.duration) * 100;
    ...
    Object.assign(frameControls.style, { display: 'flex', justifyContent: 'center', gap: '15px' });
    ...
    const updateFrameInfo = () => {
        if (!elements.modalVideoPlayer.duration) return;
        const currentFrame = Math.floor(elements.modalVideoPlayer.currentTime * frameRate);
    ...
    const handleFrameStep = (direction) => {
        ...
        const currentFrame = Math.floor(elements.modalVideoPlayer.currentTime * frameRate);
        elements.modalVideoPlayer.currentTime = Math.max(0, (currentFrame + direction) * frameDuration);
    };
    ...
    const handleKeyPress = (e) => {
        ...
        if (e.key === ' ') {
            e.preventDefault();
            elements.modalVideoPlayer.paused ? elements.modalVideoPlayer.play() : elements.modalVideoPlayer.pause();
        }
    ...
    elements.modalVideoPlayer.play().catch(err => console.warn('Autoplay prevented:', err));
}
...
export function closeModal() {
    ...
    if (elements.modalOverlay.dataset.handlersAttached === 'true') {
        ...
        if (h) {
            ...
            elements.modalVideoPlayer.removeEventListener('timeupdate', h.updateProgress);
            elements.modalVideoPlayer.removeEventListener('timeupdate', h.updateFrameInfo);
            elements.modalVideoPlayer.removeEventListener('loadedmetadata', h.updateFrameInfo);
